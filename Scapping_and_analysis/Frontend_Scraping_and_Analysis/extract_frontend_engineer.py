...
url = 'https://www.naukri.com/frontend-engineer-jobs'
driver.get(url)
time.sleep(4)
...
for k in range(1, 260):
    if k == 1:
        ...
        url = 'https://www.naukri.com/frontend-engineer-jobs-'+str(k)
        print(url)
        driver.get(url)
        time.sleep(3)
        a = driver.find_elements_by_class_name("title.fw500.ellipsis")
...
file_to_write = open("frontend_engineer.pickle", "wb")

pickle.dump(dict_links, file_to_write)

df = pd.read_pickle(r'frontend_engineer.pickle')
print(len(df))
