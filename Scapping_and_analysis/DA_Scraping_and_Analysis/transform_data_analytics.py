...
def mult_requirements():
    dff = pd.read_pickle(r'data_analytics.pickle')
    print("*********************************************************************")
    print(len(dff))
    print("*********************************************************************")

    # 14840
    df = list(dff.values())[:1001]
    count = 0
    first_dict = {}
    second_dict = {}
    third_dict = {}
    ...
    for i in df:
        if i == 'https://www.naukri.com/job-listings-kannada-speaking-regional-manager-for-political-survey-for-2-years-randstad-india-pvt-ltd-bangalore-bengaluru-2-to-6-years-270421005024?src=jobsearchDesk&sid=16201128541896242&xp=15&px=51':
            continue
        count += 1
        print(count)
    ...
    with open("first_dict_da1.pickle", "wb") as h1:
        pickle.dump(first_dict, h1)
    with open("second_dict_da1.pickle", "wb") as h2:
        pickle.dump(second_dict, h2)
    with open("third_dict_da1.pickle", "wb") as h3:
        pickle.dump(third_dict, h3)
    with open("fourth_dict_da1.pickle", "wb") as h4:
        pickle.dump(fourth_dict, h4)
    with open("fifth_dict_da1.pickle", "wb") as h5:
        pickle.dump(fifth_dict, h5)

    df1 = pd.read_pickle(r'first_dict_da1.pickle')
    print(len(df1))
    df2 = pd.read_pickle(r'second_dict_da1.pickle')
    print(len(df2))
    df3 = pd.read_pickle(r'third_dict_da1.pickle')
    print(len(df3))
    df4 = pd.read_pickle(r'fourth_dict_da1.pickle')
    print(len(df4))
    df5 = pd.read_pickle(r'fifth_dict_da1.pickle')
    print(len(df5))
