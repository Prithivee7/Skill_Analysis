...
# Split the experience required for the job
...
def mult_requirements():
    dff = pd.read_pickle(r'machine_learning.pickle')
    print("*********************************************************************")
    print(len(dff))
    print("*********************************************************************")
    df = list(dff.values())[6001:]
    count = 0
    first_dict = {}
    second_dict = {}
    third_dict = {}
    ...
    for i in df:
        count += 1
        if i == 'https://www.naukri.com/job-listings-hiring-in-data-analytics-3-years-of-experience-in-alteryx-kpmg-global-services-private-limited-bangalore-bengaluru-3-to-7-years-180321002367?src=jobsearchDesk&sid=16200367062618516&xp=3&px=236':
            continue
        print(count)
    ...
    with open("first_dict_ml7.pickle", "wb") as h1:
        pickle.dump(first_dict, h1)
    with open("second_dict_ml7.pickle", "wb") as h2:
        pickle.dump(second_dict, h2)
    with open("third_dict_ml7.pickle", "wb") as h3:
        pickle.dump(third_dict, h3)
    with open("fourth_dict_ml7.pickle", "wb") as h4:
        pickle.dump(fourth_dict, h4)
    with open("fifth_dict_ml7.pickle", "wb") as h5:
        pickle.dump(fifth_dict, h5)

    df1 = pd.read_pickle(r'first_dict_ml7.pickle')
    print(len(df1))
    df2 = pd.read_pickle(r'second_dict_ml7.pickle')
    print(len(df2))
    df3 = pd.read_pickle(r'third_dict_ml7.pickle')
    print(len(df3))
    df4 = pd.read_pickle(r'fourth_dict_ml7.pickle')
    print(len(df4))
    df5 = pd.read_pickle(r'fifth_dict_ml7.pickle')
    print(len(df5))
